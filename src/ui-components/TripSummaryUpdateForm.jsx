...
import { getTripSummary } from "../graphql/queries";
import { updateTripSummary } from "../graphql/mutations";
const client = generateClient();
export default function TripSummaryUpdateForm(props) {
  const {
    id: idProp,
    tripSummary: tripSummaryModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = tripSummaryRecord
      ? { ...initialValues, ...tripSummaryRecord }
      : initialValues;
    setTotalDistance(cleanValues.totalDistance);
    setCancellationCharge(cleanValues.cancellationCharge);
    setTripStatus(cleanValues.tripStatus);
    setTimeCompleted(cleanValues.timeCompleted);
    setTotalCharge(cleanValues.totalCharge);
    setErrors({});
  };
  const [tripSummaryRecord, setTripSummaryRecord] =
    React.useState(tripSummaryModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getTripSummary.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTripSummary
        : tripSummaryModelProp;
      setTripSummaryRecord(record);
    };
    queryData();
  }, [idProp, tripSummaryModelProp]);
  React.useEffect(resetStateValues, [tripSummaryRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTripSummary.replaceAll("__typename", ""),
            variables: {
              input: {
                id: tripSummaryRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TripSummaryUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || tripSummaryModelProp)}
